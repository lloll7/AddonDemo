    ...
    DeviceStore.prototype.ensureColumn = function (columnName, columnType) {
        ...
        this.db.all("PRAGMA table_info(devices)", function (err, rows) {
            ...
            var exists = Array.isArray(rows)
                ? rows.some(function (r) { return r.name === columnName; })
                : false;
            if (!exists) {
                _this.db.run("ALTER TABLE devices ADD COLUMN ".concat(columnName, " ").concat(columnType), function (alterErr) {
                    if (alterErr) {
                        console.error("\u6DFB\u52A0\u5217 ".concat(columnName, " \u5931\u8D25:"), alterErr.message);
                    }
                    else {
                        console.log("\u5DF2\u4E3A devices \u8868\u6DFB\u52A0\u7F3A\u5931\u5217: ".concat(columnName));
                    }
                });
            }
        });
